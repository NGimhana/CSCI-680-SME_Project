...
def csv2dict_new(csv_question_path, csv_documents_path):
    ...
    bug_report_data = pd.read_csv(csv_question_path)
    document_data = pd.read_csv(csv_documents_path)
    bug_id_file_url = bug_report_data.merge(document_data, how='left', on=['bug_id'])

    dict_list = []
    # Iterate through bug reports
    for i in range(len(bug_report_data)):
        line = {}
        line['id'] = str(bug_report_data.iloc[i,0])
        line['bug_id'] = str(bug_report_data.iloc[i,0])
        line["summary"] = bug_report_data.iloc[i,1]
        line["description"] = bug_report_data.iloc[i,1]
        line["report_time"]= "datetime.datetime(2012, 7, 3, 3, 39, 25)"
        line["report_timestamp"] = '1341300000'
        line["status"] = 'resolved fixed'
        line["commit"] = "5da5952"
        line["commit_timestamp"] = "1389970000"
        line["raw_text"]=bug_report_data.iloc[i,1]
        line["files"] = []
...
def get_all_source_code_new(start_dir):
    iterate_files(start_dir)
    return files
        ...
        if filename.is_file():
            if filename.name.endswith(".java"):
                ...
                with open(filename, 'r', newline='', encoding='UTF-8',errors='ignore') as f:
                    ...
                    files[filename.path] = document_content
...
def helper_collections(samples, only_rvsm=True):
    ...
    # sample_dict # 'file_url' : [rvsm value, ....]
    # bug_reports # a dictionary of bug repots...see utilcsv2dic_new method to get an idea of format.
    # br2files_dict # bug_id: [correct file urls...]
...
def topk_accuarcy(test_bug_reports, sample_dict, br2files_dict, clf=None):
    ...
    for bug_report in test_bug_reports:
        ...
        for i in range(1, 11):
            ...
            top_k_predicted_files = np.array(corresponding_files)[max_indices]
